    ...
    <>
      ...
      <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-6 mb-8">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Button data-testid="dashboard-filter-btn" className="bg-[#CCFF00] text-black hover:bg-[#b8e600] font-semibold rounded-lg h-10 px-5 text-sm gap-2">
            <SlidersHorizontal className="w-4 h-4" />
            Filter
          </Button>
          <button className="h-10 px-4 bg-[#111111] border border-white/10 rounded-lg text-white/40 text-sm hover:border-white/20 hover:text-white/60 transition-colors">Marke (z.B. VW)</button>
          <button className="h-10 px-4 bg-[#111111] border border-white/10 rounded-lg text-white/40 text-sm hover:border-white/20 hover:text-white/60 transition-colors">Min Preis</button>
          <button className="h-10 px-4 bg-[#111111] border border-white/10 rounded-lg text-white/40 text-sm hover:border-white/20 hover:text-white/60 transition-colors">Max Preis</button>
        </div>
      </div>
          ...
          return (
            <motion.div key={card.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} data-testid={`kpi-card-${i}`}
              ...
              <div>
                ...
                <div className="font-['Space_Grotesk'] text-4xl font-bold text-white tracking-tighter">{card.value}</div>
                <div className="flex items-center gap-2 mt-3">
                  <span className={`px-2 py-0.5 rounded-md text-[10px] font-semibold ${card.badgeColor}`}>{card.badge}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} data-testid="market-chart-card" className="lg:col-span-3 bg-[#111111] border border-white/10 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg text-white/50 text-xs font-medium">Live</span>
          </div>
          <div className="h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={marketChartData}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 12 }} domain={[0, 25000]} tickFormatter={(v) => v.toLocaleString('de-DE')} />
                <Tooltip content={<ChartTooltip />} />
                <Line type="monotone" dataKey="price" stroke="#CCFF00" strokeWidth={2} dot={false} activeDot={{ r: 5, fill: '#CCFF00', stroke: '#050505', strokeWidth: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} data-testid="alert-center-card" className="lg:col-span-2 bg-[#111111] border border-white/10 rounded-2xl p-6">
          <div className="mb-5">
            ...
            {alertItems.map((alert, i) => {
              ...
              return (
                <motion.div key={alert.title} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.35 + i * 0.06 }} data-testid={`alert-item-${i}`}
                  ...
                  <div className="w-9 h-9 rounded-lg bg-white/5 flex items-center justify-center shrink-0"><Icon className="w-4 h-4 text-white/40" /></div>
                  ...
                  <span className="min-w-[30px] h-[30px] flex items-center justify-center rounded-full bg-[#CCFF00] text-black text-xs font-bold shrink-0">{alert.count}</span>
                </motion.div>
              );
            })}
          </div>
      ...
      <div className="flex items-center gap-1 mb-6 bg-[#111111] border border-white/10 rounded-xl p-1 w-fit">
        {tabs.map((tab) => (
          <button key={tab.id} data-testid={`tab-${tab.id}`} onClick={() => setActiveTab(tab.id)}
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${activeTab === tab.id ? 'bg-[#CCFF00] text-black' : 'text-white/40 hover:text-white/70'}`}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} data-testid="tuning-feed-card" className="lg:col-span-3 bg-[#111111] border border-white/10 rounded-2xl p-6">
          <div className="mb-5">
          ...
          <div className="space-y-3">
            {tuningFeed.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.45 + i * 0.06 }} data-testid={`feed-item-${i}`}
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className="flex items-center gap-1.5 text-white/25 text-xs shrink-0 ml-4"><Clock className="w-3 h-3" />{item.time}</span>
                </div>
                <div className="flex flex-wrap gap-2">
                  {item.tags.map((tag, j) => (<span key={j} className={`px-2.5 py-1 rounded-lg text-[11px] font-medium ${tag.color}`}>{tag.label}</span>))}
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }} data-testid="update-freq-card" className="lg:col-span-2 bg-[#111111] border border-white/10 rounded-2xl p-6">
          <div className="mb-5">
          ...
          <div className="h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={updateFreqData}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 12 }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 12 }} domain={[0, 12]} />
                <Tooltip content={<ChartTooltip />} />
                <Bar dataKey="updates" fill="#CCFF00" radius={[6, 6, 0, 0]} barSize={28} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
